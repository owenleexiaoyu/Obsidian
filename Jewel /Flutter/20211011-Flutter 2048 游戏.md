...
完成手势识别后，接下来就是数字块的移动，并且合并相同块（中间没有其他数字的阻隔）的计算逻辑了。这里我的算法比较挫，肯定有更简单，复杂度更低的算法来实现。我的算法是，先将一行或者一列中相同的块进行合并，然后再将所有非 0 的块移到滑动方向的那一侧。以向左滑动为例，来看看合并和移动的算法，其他方向同理：
...
滑动后，除了要移动数字块，合并相同的数字块，还有一些逻辑需要处理，这里就放在一起说，首先是当这一次滑动，如果有块的移动或合并，则在移动或合并完后，需要在空格处随机再产生一个数字，如果既没有块的移动也没有块的合并，则不会产生一个新的数字块。另一个就是当两个相同的数字块合并时，需要更新当前分数，当前分数加上合并后的数字。
