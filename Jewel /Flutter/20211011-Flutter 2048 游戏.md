...
保存历史最高分数用到了 `shared_preferences` 这个插件来保存数据，Android 平台上是使用了 SharedPreferences，这里不具体展开，看看代码的实现：
...
在进入游戏界面也就是 Game2048Page 时，需要从 SP 中将历史最高分数读取出来，展示在 Header 里：
